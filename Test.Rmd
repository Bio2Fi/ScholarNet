...
forceNetwork(Links = Links, Nodes = Nodes, Source = "source",
             ...
             charge = -10, height="100px", width="100px")
```
...
Most_Pubs <- Pubs %>% arrange(desc(year), desc(cites)) %>% dplyr::select(-cid, -pubid) %>% dplyr::filter(year %in% Years) %>% mutate(journal = str_to_title(journal)) %>% distinct() %>% slice_max(order_by = cites, n = 20)
